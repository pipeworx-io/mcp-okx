...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/instruments \
  -H 'Content-Type: application/json' \
  -d '{"instType":"SPOT"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/instruments`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
